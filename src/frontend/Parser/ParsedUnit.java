...
public class ParsedUnit {
    ...
    public final List<ParsedUnit> subUnits;
    private int index = -1;
    ...
    /**
        * 将解析单元转换为CompUnitNode对象。
        * 
        * @return 转换后的CompUnitNode对象
        */
    ...
    public StmtNode toStmtNode() {
        switch (getUnit().name) {
            ...
            case "FORTK":
                getUnit("LPARENT");
                StmtNode initStmt = null;
                getUnit("SEMICN");
                if (!curUnit.IsType("SEMICN")) {
                    initStmt = getUnit().toStmtNode();
                    getUnit("SEMICN");
                } else {
                    getUnit("SEMICN");
                }
                ExpNode loopCondFor = null;
                getUnit("SEMICN");
                if (!curUnit.IsType("SEMICN")) {
                    if(getUnit("Cond")) {
                        loopCondFor = curUnit.toExpNode();
                    }
                    getUnit("SEMICN");
                } else {
                    getUnit("SEMICN");
                }
                StmtNode updateStmt = null;
                if (!curUnit.IsType("RPARENT")) {
                    if(getUnit("ForStmt")) {
                        updateStmt = curUnit.toStmtNode();
                    }
                }
                getUnit("RPARENT");
                loopDepth++;
                StmtNode loopStmtFor = getUnit().toStmtNode();
                loopDepth--;
                return new ForLoopNode(curSymbolTable, initStmt, loopCondFor, updateStmt, loopStmtFor);
            case "WHILETK":
                getUnit("LPARENT");
